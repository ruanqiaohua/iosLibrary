...
@interface TitleViewEx()
{
    ...
    UIView                              * line;
    //
    //NSMutableArray<UIButton *>          * btnLeftArray;
    MyLinearLayout                      * llLeft;
    
    //NSMutableArray<UIButton *>          * btnRightArray;
...
-(void)initExitBtn
{
    ...
    btnExit.centerYPos.equalTo(self.centerYPos).offset(SYSTEM_TITLE_HEIGHT / 2);
...
-(void)initView
{
    ...
    [self initTitleBtn];
    //
    //btnLeftArray = [[NSMutableArray alloc] init];
    ...
    //
    //btnRightArray = [[NSMutableArray alloc] init];
...
-(void)setTitle:(NSString *)title titleFont:(UIFont *)tf titleColor:(UIColor *)tc titleAlignment:(int)ta
{
    ...
    btnTitle.centerYPos.equalTo(self.centerYPos).offset(SYSTEM_TITLE_HEIGHT / 2);
    btnTitle.heightSize.equalTo(self);
    btnTitle.myWidth = SCREEN_WIDTH / 2;
...
-(void)setExitImgStr:(NSString *)eis
{
    ...
    if (isHttp)
    {
        [btnExit sd_setImageWithURL:[NSURL URLWithString:eis] forState:UIControlStateNormal completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL)
        {
            if (image)
            {
                btnExit.mySize = PXTO6SIZE(image.size);
            }
...
-(void)addButtonWithText:(NSString *)text btnTextColor:(UIColor *)tc font:(UIFont *)font btnLoc:(int)bl param:(NSDictionary *)param execType:(int)et
{
    ...
    {
        btn.myRight = HOR_SPACE;
    }else
    {
        btn.myLeft = HOR_SPACE;
    }
    [self addButton:btn btnLoc:bl param:param execType:et];
}
