...
+ (NSMutableArray *)toHanyuPinyinStringWithNSString:(NSString *)str
                  withHanyuPinyinOutputFormat:(HanyuPinyinOutputFormat *)outputFormat
{
    NSMutableArray * resultPinyinStrBuf = [[NSMutableArray alloc] init];
    NSMutableString * unPY = nil;
    NSArray * results = nil;
    ...
    for (int i = len - 1; i > -1; i--)
    {
        ...
        results = [PinyinHelper getFormattedHanyuPinyinStringArrayWithChar:uc withHanyuPinyinOutputFormat:outputFormat];
        if (results && results.count > 0)
        {
            if (unPY.length > 0)
            {
                [resultPinyinStrBuf addObject:[NSArray arrayWithObject:unPY]];
                unPY = nil;
            }
            [resultPinyinStrBuf addObject:results];
        }else
        {
            if (unPY == nil)
            {
                unPY = [[NSMutableString alloc] initWithString:@""];
            }
            [unPY insertString:[NSString stringWithFormat:@"%C",uc] atIndex:0];
        }
    }
    if (unPY)
    {
...
+ (NSArray *)getFirstHanyuPinyinStringWithChar:(unichar)ch
                    withHanyuPinyinOutputFormat:(HanyuPinyinOutputFormat *)outputFormat
{
